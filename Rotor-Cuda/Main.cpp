...
void usage()
{
	printf("Wif key Recovery [OPTIONS...] [TARGETS]\n");
	printf("Where TARGETS is one address/xpont, or multiple hashes/xpoints file\n\n");
	printf("-wif, --wif                              : The part of the WIF key that starts with 5... or K... or L...\n");
	...
	printf("  SITE: https://github.com/phrutis/Wif-key-Recovery \n");
}
...
int main(int argc, char** argv)
{
	...
	for (unsigned int i = 0; i < args.size(); i++) {
		...
		try {
			if (optArg.equals("-h", "--help")) {
				...
				printf("  Wif key Recovery v" RELEASE "\n\n");
				printf("\n  Checking... Secp256K1\n\n");
	...
	pass[N] = 0;
	std::stringstream ss;
	ss << wif.c_str() << pass << "11111111";
	part1 = ss.str();


	int N2 = kusok - 8;
	char str2[]{ "zzzzzzzzzzzzzzzz" };
	int strN2 = 16;
	...
	ss2 << wif.c_str() << pass2 << "11111111";
	part2 = ss2.str();



	const char* base581 = part1.c_str();
	size_t base58Length1 = part1.size();
	size_t keybuflen1 = base58Length1 == 52 ? 38 : 37;
	unsigned char* keybuf1 = new unsigned char[keybuflen1];

	b58decode(keybuf1, &keybuflen1, base581, base58Length1);

	string nos2 = "";
	for (int i = 0; i < keybuflen1; i++) {
		char s[32];
		snprintf(s, 32, "%.2x", keybuf1[i]);
		string str777(s);
		nos2 = nos2 + str777;
	}
	char* cstr959 = &nos2[0];
	...
	string str0;
	string kstr0;
	string s7777 = cstr959;
	int konec = s7777.length();
	str0 = s7777.substr(2, konec - step);
	kstr0 = s7777.substr(konec - step, konec + 1);
	char* stst = &str0[0];
	rangeStart.SetBase16(stst);

	const char* base582 = part2.c_str();
	size_t base58Length2 = part2.size();
	...
	char* cstr959e = &nos3[0];

	string str20;
	string kstr2;
	string s77772 = cstr959e;
	int konec2 = s77772.length();
	str20 = s77772.substr(2, konec2 - step);
	kstr2 = s77772.substr(konec2 - step, konec2 + 1);
	char* stst20 = &str20[0];
	rangeEnd.SetBase16(stst20);;
	...
	printf("  WIF START    : %s (%d characters)\n", part1.c_str(), part1.length());
	printf("  WIF END      : %s (%d characters)\n", part2.c_str(), part2.length());
	printf("  SITE         : https://github.com/phrutis/Wif-key-Recovery \n");

	if (display == 3) {
		printf("  Global start : %s (%d bit)\n", rangeStart.GetBase16().c_str(), rangeStart.GetBitLength());
		printf("  Global end   : %s (%d bit)\n", rangeEnd.GetBase16().c_str(), rangeEnd.GetBitLength());
	}
