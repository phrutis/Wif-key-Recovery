		...
		if (rKey == 0) {
			...
			if (nbit2 > 0) {
				...
				if (value777 > 1) {
					...
					printf("\n  Rotor info   : Divide the range into CPU %d cores for fast parallel search \n", rangeDiff2.GetBase16().c_str(), rangeDiff2.GetBitLength(), nbit2);
				}
			}
		}
...
void Rotor::getCPUStartingKey(Int & tRangeStart, Int & tRangeEnd, Int & key, Point & startP)
{
	if (rKey <= 0) {
		...
		if (display > 0) {
			...
			if (trk == nbit2) {
				string cp01 = secp->GetPrivAddress(mdd, key);
				string cp02 = secp->GetPrivAddress(mdd, rangeEnd);

				printf("  CPU Core (%d) : %s -> %s \n\n", trk, cp01.c_str(), cp02.c_str());
			}
			else {
				string cp03 = secp->GetPrivAddress(mdd, key);
				string cp04 = secp->GetPrivAddress(mdd, kon);
				printf("  CPU Core (%d) : %s -> %s \n", trk, cp03.c_str(), cp04.c_str());
			}
		}
...
void Rotor::getGPUStartingKeys(Int & tRangeStart, Int & tRangeEnd, int groupSize, int nbThread, Int * keys, Point * p)
{
	if (rKey > 0) {
		...
		printf("\n  Rotor info   : Divide the range into GPU %d threads \n\n", nbThread);
		...
		for (int i = 0; i < nbThread + 1; i++) {
			...
			string pAddr0 = secp->GetPrivAddress(mdd, keys[i]);
			if (i == 0) {
				if (display > 0) {
					printf("  Thread 00000 : %s ->", pAddr0.c_str());
				}
			}
			Int dobb;
			...
			if (display > 0) {
				...
				if (i == 0) {
					string pAddr00 = secp->GetPrivAddress(mdd, dobb);
					printf(" %s \n", pAddr00.c_str());
				}
				if (i == 1) {
					string pAddr01 = secp->GetPrivAddress(mdd, tRangeStart2);
					string pAddr11 = secp->GetPrivAddress(mdd, dobb);
					printf("  Thread 00001 : %s -> %s \n", pAddr01.c_str(), pAddr11.c_str());
				}
				if (i == 2) {
					string pAddr02 = secp->GetPrivAddress(mdd, tRangeStart2);
					string pAddr22 = secp->GetPrivAddress(mdd, dobb);
					printf("  Thread 00002 : %s -> %s \n", pAddr02.c_str(), pAddr22.c_str());
				}
				if (i == 3) {
					string pAddr03 = secp->GetPrivAddress(mdd, tRangeStart2);
					string pAddr33 = secp->GetPrivAddress(mdd, dobb);
					printf("  Thread 00003 : %s -> %s \n", pAddr03.c_str(), pAddr33.c_str());
					printf("           ... : \n");
				}
				if (i == nbThread - 2) {
					string pAddr04 = secp->GetPrivAddress(mdd, tRangeStart2);
					string pAddr44 = secp->GetPrivAddress(mdd, dobb);
					printf("  Thread %d : %s -> %s \n", i, pAddr04.c_str(), pAddr44.c_str());
				}
				if (i == nbThread - 1) {
					string pAddr05 = secp->GetPrivAddress(mdd, tRangeStart2);
					string pAddr55 = secp->GetPrivAddress(mdd, dobb);
					printf("  Thread %d : %s -> %s \n", i, pAddr05.c_str(), pAddr55.c_str());
				}
				if (i == nbThread) {
					string pAddr06 = secp->GetPrivAddress(mdd, tRangeStart2);
					string pAddr66 = secp->GetPrivAddress(mdd, dobb);
					printf("  Thread %d : %s -> %s \n\n", i, pAddr06.c_str(), pAddr66.c_str());
				}
			}
						...
						if (rKey > 0) {
							...
							if (isAlive(params)) {
								...
								if (avgGpuKeyRate > 1000000000) {
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
							...
							if (avgGpuKeyRate > 1000000000) {
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [%s] [F: %d] [C: %.3f%%] [GPU: %.2f Gk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										pAddrbar.c_str(),
										nbFoundKey,
										completedPerc,
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [%s] [F: %d] [C: %.3f%%] [GPU: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										pAddrbar.c_str(),
										nbFoundKey,
										completedPerc,
						...
						if (rKey > 0) {
							...
							if (avgGpuKeyRate > 1000000000) {
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [%s] [F: %d] [C: %.3f%%] [CPU %d: %.2f Gk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										pAddrbar.c_str(),
										nbFoundKey,
										completedPerc,
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [%s] [F: %d] [C: %.3f%%] [CPU %d: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										pAddrbar.c_str(),
										nbFoundKey,
										completedPerc,
						...
						if (rKey > 0) {
							...
							if (avgGpuKeyRate > 1000000000) {
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [F: %d] [C: %.3f%%] [GPU: %.2f Gk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										nbFoundKey,
										completedPerc,
										avgGpuKeyRate / 1000000000.0,
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [F: %d] [C: %.3f%%] [GPU: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										nbFoundKey,
										completedPerc,
										avgGpuKeyRate / 1000000.0,
						...
						if (rKey > 0) {
							...
							if (avgGpuKeyRate > 1000000000) {
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [F: %d] [C: %.3f%%] [CPU %d: %.2f Gk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										nbFoundKey,
										completedPerc,
										nbit2,
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [F: %d] [C: %.3f%%] [CPU %d: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										nbFoundKey,
										completedPerc,
										nbit2,
						...
						if (rKey > 0) {
							...
							if (avgGpuKeyRate > 1000000000) {
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [%s] [F: %d] [Y:%03llu D:%03llu] [C: %.3f%%] [GPU: %.2f Gk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										pAddrbar.c_str(),
										nbFoundKey,
										years88,
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [%s] [F: %d] [Y:%03llu D:%03llu] [C: %.3f%%] [GPU: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										pAddrbar.c_str(),
										nbFoundKey,
										years88,
						...
						if (rKey > 0) {
							...
							if (avgGpuKeyRate > 1000000000) {
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [%s] [F: %d] [Y:%03llu D:%03llu] [C: %.3f%%] [CPU %d: %.2f Gk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										pAddrbar.c_str(),
										nbFoundKey,
										years88,
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [%s] [F: %d] [Y:%03llu D:%03llu] [C: %.3f%%] [CPU %d: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										pAddrbar.c_str(),
										nbFoundKey,
										years88,
							...
							if (rKey > 0) {
								...
								if (avgGpuKeyRate > 1000000000) {
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [%s] [F: %d] [D:%03llu %02llu:%02llu:%02llu] [C: %.3f%%] [GPU: %.2f Gk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											pAddrbar.c_str(),
											nbFoundKey,
											days88,
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [%s] [F: %d] [D:%03llu %02llu:%02llu:%02llu] [C: %.3f%%] [GPU: %.2f Mk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											pAddrbar.c_str(),
											nbFoundKey,
											days88,
							...
							if (rKey > 0) {
								...
								if (avgGpuKeyRate > 1000000000) {
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [%s] [F: %d] [D:%03llu %02llu:%02llu:%02llu] [C: %.3f%%] [CPU %d: %.2f Gk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											pAddrbar.c_str(),
											nbFoundKey,
											days88,
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [%s] [F: %d] [D:%03llu %02llu:%02llu:%02llu] [C: %.3f%%] [CPU %d: %.2f Mk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											pAddrbar.c_str(),
											nbFoundKey,
											days88,
							...
							if (rKey > 0) {
								...
								if (avgGpuKeyRate > 1000000000) {
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [%s] [F: %d] [%02llu:%02llu:%02llu] [C: %.3f%%] [GPU: %.2f Gk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											pAddrbar.c_str(),
											nbFoundKey,
											hours88,
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [%s] [F: %d] [%02llu:%02llu:%02llu] [C: %.3f%%] [GPU: %.2f Mk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											pAddrbar.c_str(),
											nbFoundKey,
											hours88,
							...
							if (rKey > 0) {
								...
								if (avgGpuKeyRate > 1000000000) {
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [%s] [F: %d] [%02llu:%02llu:%02llu] [C: %.3f%%] [CPU %d: %.2f Gk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											pAddrbar.c_str(),
											nbFoundKey,
											hours88,
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [%s] [F: %d] [%02llu:%02llu:%02llu] [C: %.3f%%] [CPU %d: %.2f Mk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											pAddrbar.c_str(),
											nbFoundKey,
											hours88,
						...
						if (rKey > 0) {
							...
							if (avgGpuKeyRate > 1000000000) {
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [F: %d] [Y:%03llu D:%03llu] [C: %.3f%%] [GPU: %.2f Gk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										nbFoundKey,
										years88,
										days88,
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [F: %d] [Y:%03llu D:%03llu] [C: %.3f%%] [GPU: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										nbFoundKey,
										years88,
										days88,
						...
						if (rKey > 0) {
							...
							if (avgGpuKeyRate > 1000000000) {
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [F: %d] [Y:%03llu D:%03llu] [C: %.3f%%] [CPU %d: %.2f Gk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										nbFoundKey,
										years88,
										days88,
								...
								if (isAlive(params)) {
									...
									string pAddrbar = secp->GetPrivAddress(mdd, rhex);
									printf("\r  [%s] [F: %d] [Y:%03llu D:%03llu] [C: %.3f%%] [CPU %d: %.2f Mk/s] [T: %s]  ",
										toTimeStr(t1, timeStr),
										nbFoundKey,
										years88,
										days88,
							...
							if (rKey > 0) {
								...
								if (avgGpuKeyRate > 1000000000) {
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [F: %d] [D:%03llu %02llu:%02llu:%02llu] [C: %.3f%%] [GPU: %.2f Gk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											nbFoundKey,
											days88,
											hours88,
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [F: %d] [D:%03llu %02llu:%02llu:%02llu] [C: %.3f%%] [GPU: %.2f Mk/s] [T: %s]   ",
											toTimeStr(t1, timeStr),
											nbFoundKey,
											days88,
											hours88,
							...
							if (rKey > 0) {
								...
								if (avgGpuKeyRate > 1000000000) {
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [F: %d] [D:%03llu %02llu:%02llu:%02llu] [C: %.3f%%] [CPU %d: %.2f Gk/s] [T: %s]   ",
											toTimeStr(t1, timeStr),
											nbFoundKey,
											days88,
											hours88,
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [F: %d] [D:%03llu %02llu:%02llu:%02llu] [C: %.3f%%] [CPU %d: %.2f Mk/s] [T: %s]   ",
											toTimeStr(t1, timeStr),
											nbFoundKey,
											days88,
											hours88,
							...
							if (rKey > 0) {
								...
								if (avgGpuKeyRate > 1000000000) {
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [F: %d] [%02llu:%02llu:%02llu] [C: %.3f%%] [GPU: %.2f Gk/s] [T: %s]   ",
											toTimeStr(t1, timeStr),
											nbFoundKey,
											hours88,
											minutes88,
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [F: %d] [%02llu:%02llu:%02llu] [C: %.3f%%] [GPU: %.2f Mk/s] [T: %s]   ",
											toTimeStr(t1, timeStr),
											nbFoundKey,
											hours88,
											minutes88,
							...
							if (rKey > 0) {
								...
								if (avgGpuKeyRate > 1000000000) {
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [F: %d] [%02llu:%02llu:%02llu] [C: %.3f%%] [CPU %d: %.2f Gk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											nbFoundKey,
											hours88,
											minutes88,
									...
									if (isAlive(params)) {
										...
										string pAddrbar = secp->GetPrivAddress(mdd, rhex);
										printf("\r  [%s] [F: %d] [%02llu:%02llu:%02llu] [C: %.3f%%] [CPU %d: %.2f Mk/s] [T: %s]  ",
											toTimeStr(t1, timeStr),
											nbFoundKey,
											hours88,
											minutes88,
